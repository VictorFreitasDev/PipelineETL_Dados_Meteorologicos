...
columns_to_normalize_datetime = ['datatime', 'sunrise', 'sunset']
...
def rename_columns(df: pd.DataFrame, columns_names:dict[str,str]) -> pd.DataFrame:
    ...
    logging.info(f"\n Colunas renomeadas: {columns_names} - {len(df.columns)} colunas restantes")
    return df
...
def normalize_datetime_columns(df: pd.DataFrame, columns_names:list[str]) -> pd.DataFrame:
    for name in columns_names:
        ...
        logging.info(f"\n Colunas de data normalizadas: {columns_names} - {len(df.columns)} colunas     restantes")
    return df
...
def data_transformations():
    ...
    logging.info(f"\n Transformações concluídas - {len(df.columns)} colunas restantes")
    return df